...
- **Components**:
  ...
  2. **Título (h2)** — `Empresas que confían en Inntuitivo 🚀`. Audiowide 400, `font-size: 34px`, `line-height: 1.3`, color `#FFFFFF`, centrado, `text-wrap: balance`.
...
- **Marquee**: `@keyframes marquee { from { transform: translate3d(0,0,0) } to { transform: translate3d(-50%,0,0) } }`, `animation: marquee 46s linear infinite`. El `-50%` funciona porque el track contiene la lista duplicada exactamente dos veces: el loop es invisible.
- **Pausa en hover**: al hacer hover sobre el wrapper, `animation-play-state: paused` en el track.
- **Hover de logo**: `opacity` de `.55` → `1` y `transform: scale(1.05)`, transición `.35s ease` en opacity y transform.
...
- Duración de animación: 46s (marquee), .35s (hover), 4s (twinkle).

### Props del prototipo
| Prop | Default | Rango |
|---|---|---|
| `titulo` | Empresas que confían en Inntuitivo 🚀 | texto |
| `subtitulo` | Empresas, startups, inversionistas, programas… 🪐 | texto |
| `tema` | Oscuro | Oscuro / Claro |
| `velocidad` | 46s | 15–90s |
...
Archivos (mismo nombre en ambas carpetas): `aws.png`, `microsoft-founders-hub.png`, `google-for-startups.png`, `openai.png`, `claude.png`, `hubspot-for-startups.png`, `neural-coders.png`, `mural.png`, `scrumstudy.png`, `advice-legal.png`.
...
Orden en la cinta: AWS → Microsoft Founders Hub → Google for Startups → OpenAI → Claude → HubSpot → Neural Coders → Mural → SCRUMstudy → Advice Legal.
